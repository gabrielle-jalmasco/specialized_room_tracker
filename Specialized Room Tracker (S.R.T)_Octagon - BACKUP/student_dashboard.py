...
class StudentDashboard(QWidget):
    ...
    def edit_reservation(self):
        # index of the row the user clicked on
        selected_row_index_integer = self.table.currentRow()

        if selected_row_index_integer < 0:
            QMessageBox.warning(self, "No Selection", "Please select a reservation to edit.")
            return

        # KUNIN reservation_id FIRST COUMN
        selected_item_object = self.table.item(selected_row_index_integer, 0)
        reservation_id_string = selected_item_object.text()

        # 'created_at' CHECK 'reservation_id'
        sql_query_string = "SELECT created_at FROM reservations WHERE reservation_id = %s"
        query_parameters_tuple = (reservation_id_string,)
        
        database_results_list = self.db_manager.fetch_all(sql_query_string, query_parameters_tuple)

        # NAHANAP BA YUNG RESERVATION? (CHECK LOGIC)
        if len(database_results_list) == 0:
            return

        # Get first result dictionary
        first_result_dictionary = database_results_list[0]
        creation_time_value = first_result_dictionary['created_at']

        if type(creation_time_value) == str:
            # Convert string "2023-01-01 12:00:00" to a datetime object
            creation_datetime_object = datetime.strptime(creation_time_value, "%Y-%m-%d %H:%M:%S") # YEAR | MONTH | DAY | HOUR | MINUTES | SECONDS FORMAT
        else:
          creation_datetime_object = creation_time_value

        # PRESENT TIME
        current_datetime_now = datetime.now()

        # DIFFERENCE NG DATETIME NGAYON SA DATETIME NA GINAWA (PRESENT TIME - CREATION TIME)
        time_difference_object = current_datetime_now - creation_datetime_object
        difference_in_seconds = time_difference_object.total_seconds()
        
        # 5 minutes * 60 seconds = 300 seconds
        # 
        # FOR TEST PURPOSES CHANGE LATER SA DEMO DAY TO 10 SECONDS TIME INTERVAL
        limit_in_seconds = 300

        if difference_in_seconds > limit_in_seconds:
            formatted_time_string = creation_datetime_object.strftime('%I:%M %p')
            
            message_line_1 = "You can only edit a reservation within 5 minutes of creation.\n"
            message_line_2 = "Created at: " + formatted_time_string + "\n"
            message_line_3 = "Please contact an administrator if you need to change this."
            full_error_message = message_line_1 + message_line_2 + message_line_3
            
            QMessageBox.warning(self, "Action Denied", full_error_message)
            return

        edit_dialog_window = ReservationDialog(self.db_manager, self.user, self.is_currently_dark, self, res_id=reservation_id_string)
        dialog_result = edit_dialog_window.exec()
...
class ReservationDialog(QDialog):
    ...
    def submit(self):
        ...
        if self.res_id:
            query = """
                UPDATE reservations 
                SET room_id=%s, full_name=%s, course_section=%s, 
                    reservation_type=%s, start_time=%s, end_time=%s, activity_description=%s
                WHERE reservation_id=%s
            """
            params = (self.room_combo.currentData(), self.name_input.text(), 
                      self.course_input.text(), self.type_input.currentText(), 
                      dt_str, end_dt_str, self.purpose_input.text(), self.res_id)
